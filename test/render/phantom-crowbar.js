...
if (system.args.length < 4) {
  console.log('Usage: phantom-crowbar.js <some URL> <selector> <directory>');
  phantom.exit();
}
...
page.open(address, function(status) {
  if (status !== 'success') {
    ...
    var els = page.evaluate(extract, selector);

    // write each svg to a file, and notify the main process
    els.forEach(function(svg) {
      svg.filename = directory + svg.filename;
      fs.write(svg.filename, svg.source, 'w');
      system.stdout.write(svg.filename + '","');
    })
...
function extract(selector) {
  ...
  var svgs = [].map.call(els, function(el) {
    return {
